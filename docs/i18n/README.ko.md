...
<p align="center"><b>하나의 명령어. 어떤 Provider든. Claude CLI에 대한 완전한 제어.</b></p>

---

<p align="center">
환경 변수와 설정 파일을 뒤적이는 것은 그만하세요.<br/>
Claudy를 사용하면 단 하나의 명령어로 Anthropic, Z.AI, OpenRouter, Ollama, 커스텀 엔드포인트 간에 전환할 수 있으며, 자격증명, 설정 모드, Claude 프레임워크를 프로필별로 깔끔하게 분리해 관리합니다.
</p>

<p align="center">
<b>멀티 Provider · 설정 격리 · Channel 브릿지 · 로컬 에이전트 브릿지 · 사용량 분석</b>
</p>

---

<p align="center"><b>Claude CLI를 위한 멀티 Provider 런처.</b></p>
...
> Claudy는 Go 기반의 Claude CLI 멀티 Provider 런처인 [Clother](https://github.com/jolehuit/clother)에서 영감을 받았습니다. Z.AI Provider가 가장 철저하게 테스트되었습니다. 다른 Provider에서 문제가 발생하면 [이슈를 등록해 주세요](https://github.com/epicsagas/claudy/issues).

| Provider | 상태 | 비고 |
|---|---|---|
| 빌트인 (Anthropic) | ✅ 테스트 완료 | 기본값 |
| Z.AI | ✅ 테스트 완료 | |
| OpenRouter 별칭 | ⚠️ 실험적 | 아직 완전히 테스트되지 않음 — GitHub에 이슈를 등록해 주세요 |
...
```bash
claudy mcp run        # MCP 서버 시작 (Claude Code에서 호출)
claudy mcp install    # Claude Code 설정에 claudy를 MCP 서버로 등록
claudy mcp uninstall  # Claude Code MCP 설정에서 claudy 제거
```

`claudy mcp install`을 실행하면 `~/.claude/settings.json`에 자동으로 등록됩니다. `claudy mode create <name>`으로 Mode를 생성하면 Mode의 설정 파일에도 등록됩니다. 수동 설정은 필요 없습니다.
...
claudy analytics sync-pricing      # models.dev와 Anthropic 가격 페이지에서 모델 가격 동기화
claudy analytics recalculate       # 최신 가격 데이터로 모든 비용 재계산
```

### Claude Code 내부에서: `/analytics-insights`

사용량을 분석하는 가장 빠른 방법은 Claude Code 내부에서 직접 하는 것입니다. `analytics-insights` 스킬은 자동으로 사용 가능합니다 — 자연스럽게 질문하세요:
...
Claude가 `claudy analytics insights`를 실행하고, JSON을 분석하여 다음을 포함한 구조화된 리포트를 반환합니다:

- **비용 추세** — 일별/주별 지출 및 급증 감지
- **모델 분포** — 어떤 모델을 사용하는지, 세션당 비용은 얼마인지
- **도구 패턴** — 가장 많이 사용하는 도구, 오류율, 효율성 관찰
- **캐시 성능** — 적중률 및 예상 절감액
- **실행 가능한 권장 사항** — "간단한 작업은 turbo로 라우팅"과 같은 구체적인 제안과 예상 절감 금액

출력 예시(원시 데이터는 [`docs/examples/analytics-insights-sample.json`](docs/examples/analytics-insights-sample.json) 참조):
...
수동 명령어도, 컨텍스트 전환도 없습니다. Claude에게 사용량에 대해 물어보고 즉시 답변을 받으세요.
