...
<picture>
  ...
  <img alt="Claudy 功能介绍" src="../assets/features-2048.png" width="100%">
</picture>
...
| | 功能 | 为什么重要 |
|--|------|----------|
| 🔄 | 多 Provider 启动 | 一条命令在 Anthropic、Z.AI、OpenRouter、Ollama 和自定义端点之间切换 |
| 📦 | Config Mode | 按模式隔离 `CLAUDE.md`、设置、技能和代理 — 互不干扰 |
| 🔗 | Agent MCP 桥接 | 从 Claude Code 委托任务给 Gemini、Codex、Aider 等 20+ 代理 |
| 💬 | Channel 桥接 | 运行 Telegram、Slack 和 Discord 机器人，支持交互式权限提示 |
| 📊 | 使用分析 | 通过本地 Tauri 仪表板追踪令牌使用量、成本和工具模式 |
| 🔐 | 安全进程控制 | SIGINT/SIGTERM 转发、原子配置写入、0600 凭证存储 |
| 🛠️ | 运维 UX | 安装、更新、卸载、诊断、连接测试 — 一个二进制文件搞定 |
...
- `claudy mcp <subcommand>`：管理代理桥接的 MCP 服务器。
...
claudy mcp install    # 将 claudy 注册为 Claude Code 设置中的 MCP 服务器
claudy mcp uninstall  # 从 Claude Code MCP 设置中移除 claudy
```

`claudy mcp install` 会自动将自身注册到 `~/.claude/settings.json`。使用 `claudy mode create <name>` 创建 Mode 时，也会注册到该 Mode 的设置文件中。无需手动配置。
...
claudy analytics sync-pricing      # 从 models.dev 和 Anthropic 定价页同步模型定价
...
分析使用情况最快的方式是直接在 Claude Code 内部进行。`analytics-insights` 技能自动可用 — 只需自然地提问：
...
Claude 运行 `claudy analytics insights`，分析 JSON，并返回结构化报告，包含：

- **成本趋势** — 日/周支出与激增检测
- **模型分布** — 您使用的模型及其每次会话的成本
- **工具模式** — 最常用工具、错误率、效率观察
- **缓存性能** — 命中率和预计节省金额
- **可操作建议** — 具体建议，如"将简单任务路由到 turbo"，附带预计节省金额

输出示例（原始数据见 [`docs/examples/analytics-insights-sample.json`](docs/examples/analytics-insights-sample.json)）：
...
无需手动命令，无需切换上下文。向 Claude 询问您的使用情况，即刻获得答案。
