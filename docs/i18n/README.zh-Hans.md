...
> Claudy 受到 [Clother](https://github.com/jolehuit/clother)（一个基于 Go 的 Claude CLI 多 Provider 启动器）的启发。**只有 Z.AI Provider 经过了完整测试**。所有其他替代 Provider 都是实验性的且未经测试——使用时请自行承担风险。

| Provider | 状态 | 备注 |
|---|---|---|
| 内置 (Anthropic) | ✅ 已测试 | 默认 |
| Z.AI | ✅ 已测试 | 完全验证 |
| OpenRouter 别名 | ⚠️ 实验性 | 未测试——使用时请自行承担风险 |
| Ollama | ⚠️ 实验性 | 未测试——使用时请自行承担风险 |
| 自定义端点 | ⚠️ 实验性 | 未测试——使用时请自行承担风险 |
...
- `config.json`：Provider + Channel + 代理配置。
...
- **代理超时**：在 `config.json` 的 agents 字段中增加超时时间（默认：120 秒）。
