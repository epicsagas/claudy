...
|--|---------------|------------------------|
| 🔄 | Lancement multi-fournisseur | Basculez entre Anthropic, Z.AI, OpenRouter, Ollama et endpoints personnalisés en une commande |
| 📦 | Config modes | Isolez `CLAUDE.md`, paramètres, skills et agents par mode — sans contamination croisée |
| 🔗 | Agent MCP bridge | Déléguez des tâches de Claude Code à Gemini, Codex, Aider et 20+ autres agents |
| 💬 | Channel bridge | Exécutez des bots Telegram, Slack et Discord avec des invites de permissions interactives |
| 📊 | Analytics d'utilisation | Suivez l'utilisation des tokens, les coûts et les patterns d'outils dans un dashboard Tauri local |
| 🔐 | Contrôle de processus sûr | Transfert SIGINT/SIGTERM, écritures de config atomiques, stockage d'identifiants 0600 |
| 🛠️ | UX opérationnel | Installation, mise à jour, désinstallation, diagnostic, test de connexion — un seul binaire |
...
### Installer depuis le code source Git
