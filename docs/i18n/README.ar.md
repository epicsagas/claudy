...
| 🔀 | استمرارية الجلسة عبر المزودين | إصلاح جلسات Z.AI/GLM تلقائيًا لاستئنافها مع واجهة Anthropic API دون انقطاع |
| 🫴 | تسليم الجلسة عبر واجهات سطر الأوامر | نفدت الحصة في Codex أو Antigravity؟ يستخرج `claudy handoff` الجلسة ويُنشئ جلسة Claude جديدة بها |
| 🛡️ | حارس حركة الصادر | يضع `claudy --guard <profile>` وكيلًا محليًا يحجب كتل الصور ويُخفّي الأسرار المتسرّبة قبل مغادرة أي بيانات للجهاز |
| 🛠️ | تجربة تشغيلية | تثبيت، تحديث، إزالة، تشخيص، اختبار اتصال — كل شيء من ملف ثنائي واحد |

## نفدت الحصة في Codex أو Antigravity؟ سلّم الجلسة إلى Claude

الاستئناف عبر المزودين يعمل داخل Claude CLI فقط — فقد يحتفظ Codex وAntigravity بمخازن جلسات خاصة بهما بصيغ مختلفة، ولا يمكن لـ`--resume` تحميلها. عند نفاد حصة هذه الواجهات، يستخرج `claudy handoff` ملخصًا للمحادثة من الجلسة الأجنبية ويُنشئ به جلسة Claude **جديدة**:

```bash
# تفاعلي — يعرض جلسات codex + agy للدليل الحالي
claudy handoff

# ضمن ملف شخصي محدد (أي بادئة ملف شخصي تعمل)
claudy zai handoff

# أحدث جلسة عبر كلا الواجهتين، بدون منتقي
claudy zai handoff -c

# الاختيار من بين أحدث 5 جلسات
claudy handoff -r

# حصر الواجهة ببرنامج واحد؛ معاينة الملخص دون تشغيل
...
يعكس `-c` سلوك `--continue` في Claude (الأحدث) ويعكس `-r` سلوك `--resume` (الاختيار) — لكن فوق مخازن الجلسات الأجنبية. تُمرَّر `--yolo` وأي أعلام أخرى غير المعروفة إلى جلسة Claude كما هي حرفيًا.

**ما الذي يُستخرج:** موجّهات المستخدم الأصلية (حرفيًا مع سقف)، وردود المساعد ونشاط الأدوات (مقتصرة على أسطر واحدة)، والحالة النهائية للمساعد، ومسار مساحة العمل — تُصاغ في موجّه واحد ضمن حد 16 KiB. تُقرأ ملفات rollout الخاصة بـCodex قراءة أصلية. أما قاعدة بيانات جلسات Antigravity فصيغتها غير موثّقة، لذا يقرأها claudy بأفضل جهد (مسح سلاسل protobuf عام مع فهرس سجل الموجّهات) ويعود إلى الموجّهات فقط عند تغيّر التنسيق.

تبدأ جلسة Claude الجديدة في نفس دليل العمل مع الملخص كأول رسالة — تراجع ملخص المحادثة مع الحالة الحالية للمستودع (`git status`/`git diff` مقترحان عليها) وتكمل من هناك. هذا تسليم سياق لا استئناف بتًا-ببتًا: توقّع من النموذج إعادة التحقق من التفاصيل بدل إعادة تشغيلها.

**الأعلام:** `[codex|agy]` مصدر موضعي (يفحص الاثنين عند الحذف) · `-c, --continue` (أحدث جلسة) · `-r, --resume` (اختيار من بين أحدث 5) · `--id <session-id>` (تخطي المنتقي) · `--cwd <dir>` (الافتراضي: الدليل الحالي؛ يعود إلى كل الجلسات عند عدم وجود تطابق) · `--profile <p>` (أو بادئة ملف شخصي: `claudy zai handoff`) · `--print` (stdout فقط) · `--yolo` (تمرير `--dangerously-skip-permissions` إلى Claude؛ بقية الأعلام غير المعروفة تُمرَّر حرفيًا).

## أبقِ الأسرار والصور بعيدًا عن البوابة: `--guard`

ترى بوابة الطرف الثالث كل شيء — الجلسة بأكملها نصًا واضحًا، وأي ملف ثنائي يحتاج النموذج إلى رؤيته (لقطات الشاشة، قراءات الصور المحلية) يُرفع بصيغة base64. بعض البوابات تعيد رفع هذه الملفات إلى مخازن التخزين الخاصة بها دون إخبارك. يضع `--guard` وكيلًا عكسيًا محليًا بين Claude CLI والمزوّد: تُفحص أجسام الطلبات وتُعاد كتابتها **قبل مغادرتها الجهاز**، وتعود الاستجابات دون مساس.

```bash
claudy --guard zai          # أي موضع يعمل: claudy zai --guard كذلك
claudy --guard zai work     # يتحد مع الأوضاع كالمعتاد
```

عند البدء يطبع claudy عنوان الاستماع للوكيل:
...
| الكشف | الإجراء الافتراضي | التفصيل |
|---|---|---|
| كتل محتوى `{"type":"image"}` (محتوى الرسائل، مصفوفات `system`، محتوى `tool_result` المتداخل) | تُستبدل بعنصر نصي بديل | الملف الثنائي لا يصل إلى البوابة أصلًا، فلا يمكن إعادة رفعه |
| مفاتيح API — `sk-*`، `AKIA…`، `gh[posu]_…`، Slack `xox[bpas]-…` | يُستبدل الرمز بـ `[REDACTED:<kind>]` | مجموعات المحارف تستبعد علامات الاقتباس/بنية JSON، فيبقى الطلب قابلًا للتحليل |
| رموز `Bearer`/`Basic`، أزواج `key=value` | يُستبدل الرمز مع حفظ البادئة | الحدود الدنيا للأطوال تمنع الإيجابيات الكاذبة في النثر |
| الأجسام غير JSON أو غير القابلة للتحليل | تمرير + تحذير في السجل | fail-open: لا يحجب أبدًا بسبب قيود الماسح |

الطلبات النظيفة تُمرَّر مطابقة بالبايت. يُسجَّل كل طلب في `~/.claudy/guard/ledger.jsonl` (الطريقة، المسار، مضيف المنبع، عدادات البايتات، النتائج) — المعاينات مقنَّعة (`sk-a****f789`)، ولا تُكتب مادة الأسرار الخام أبدًا.

تقع **السياسة** تحت `guard:` في `config.yaml`:

```yaml
guard:
  strip_images: true          # استبدال كتل الصور قبل الصادر
  on_secret: redact           # allow | redact | warn | block
  trusted_providers: [native] # النتائج على غيرها تضيف نصيحة إعادة توجيه
```

يرفض `block` الطلب بخطأ 400 ولا يتصل بالمنبع إطلاقًا. يحرّك `trusted_providers` نصيحة أحادية (stderr + السجل) تقترح مزوّدًا موثوقًا عند كشف محتوى حساس على مزوّد غير موثوق — فتبديل المزوّد منتصف الجلسة مستحيل، لذا تبقى اقتراحًا.

**القيود:** الرؤية (vision) معطّلة عمدًا تحت `--guard` (يرى النموذج عنصرًا بديلًا — هذا هو الهدف)؛ إيجابيات كاذبة نادرة قد تُخفّي سلاسل شبيهة بالمفاتيح داخل محتوى مشروع (مثل تسلسلات تشبه `AKIA` داخل كتل base64 كبيرة)؛ جسر القنوات وتشغيلات `handoff` لا تمر عبر الحارس بعد.
...
> المرجع الكامل: [docs/config.md](../config.md)
...
# حارس الصادر (claudy --guard <profile>)
guard:
  strip_images: true                  # الافتراضي: true
  on_secret: redact                   # allow | redact | warn | block
  trusted_providers: ["native"]       # الافتراضي: ["native"]
```
...
- `claudy [profile] handoff [codex|agy] [-c|-r]`: استئناف جلسة codex/agy نفدت حصتها في جلسة Claude جديدة.
- `claudy analytics <subcommand>`: لوحة تحليلات الاستخدام.
- `claudy session sanitize`: إصلاح الجلسات التي تحتوي على كتل thinking غير صالحة كتبها مزودون غير Anthropic.
...
## استمرارية الجلسة عبر المزودين

عند العمل مع مزود غير Anthropic (مثل Z.AI / GLM)، يحتوي ملف JSONL للجلسة على كتل thinking بتوقيع فارغ. عند استئناف تلك الجلسة عبر Anthropic API يظهر الخطأ التالي:

```
API Error: 400 Invalid `signature` in `thinking` block
```

يتعامل Claudy مع هذا بطريقتين:

**تلقائيًا (جسر القناة):** عند استئناف الخادم للجلسة، يحوّل تلقائيًا كتل thinking ذات التوقيع الفارغ إلى كتل نصية. لا حاجة لأي إجراء.

**يدويًا (CLI):** استخدم `claudy session sanitize` قبل الاستئناف بـ `claude --resume`:

```bash
# تفاعلي — اختر من قائمة الجلسات المعطوبة
claudy session sanitize

# تصفية باسم المشروع
claudy session sanitize --project book-forge

# معالجة جميع الجلسات دفعة واحدة
claudy session sanitize --all --yes
```

**ما تفعله التحويل:** تُعاد كتابة كتل thinking ذات التوقيع الفارغ كـكتل نصية عادية مع الحفاظ على محتوى الاستدلال. الكتل ذات التوقيع الصالح من Anthropic لا تتأثر.

**قيود:** تعتمد استمرارية الجلسة على توافق سجل المحادثة. قد يتسبب تغيير المزود في منتصف الجلسة في تغييرات طفيفة في السياق حتى بعد المعالجة.
...
- **`400 Invalid signature in thinking block` عند استئناف الجلسة**: الجلسة أُنشئت بمزود غير Anthropic (مثل Z.AI). شغّل `claudy session sanitize` لتحويل كتل thinking غير الصالحة ثم استأنف بشكل طبيعي.
