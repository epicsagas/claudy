...
Adicione agentes em `~/.claudy/config.json`:
...
- `config.json`: configuração de provider, Channel e agente.
...
- **Timeout do agente**: aumente o timeout no campo agents de `config.json` (padrão: 120s).
