...
- **インサイト（LLM 駆動）**: LLM 分析向けに最適化されたコンパクト JSON 形式の使用状況サマリー。コストトレンド、モデル分布、ツールパターン、キャッシュ効率、注目セッションを単一ペイロード（約 2-3K トークン）に統合します。Claude Code の `analytics-insights` スキルを使って自然言語で質問（「使用パターンを分析して」、「analyze my usage patterns」）すると、Claude がパーソナライズされた推奨を生成します。
