...
| 🔀 | Continuidad de sesión entre proveedores | Reparar automáticamente sesiones de Z.AI/GLM para reanudarlas con la API de Anthropic sin interrupciones |
| 🫴 | Traspaso de sesión entre CLIs | ¿Cuota agotada en Codex o Antigravity? `claudy handoff` extrae la sesión y siembra con ella una nueva sesión de Claude |
...
> Referencia completa: [docs/config.md](../config.md)
...
# Guardia de salida (claudy --guard <profile>)
guard:
  strip_images: true                  # predeterminado: true
  on_secret: redact                   # allow | redact | warn | block
  trusted_providers: ["native"]       # predeterminado: ["native"]
```
...
Los modos también son ideales para **frameworks y toolkits dedicados de Claude** que incluyen su propio `CLAUDE.md`, skills, agentes o ajustes — como [gstack](https://github.com/garrytan/gstack), [superpowers](https://github.com/obra/superpowers), [ecc](https://github.com/affaan-m/everything-claude-code), nuestro propio [epic-harness](https://github.com/epicsagas/epic-harness)(un plugin de Claude Code que evoluciona solo), o cualquier harness personalizado. En lugar de contaminar tu configuración predeterminada, aísla cada framework en su propio modo:
...
> **Combina bien con [epic-harness](https://github.com/epicsagas/epic-harness).** Claudy gestiona la capa operativa — cambio de proveedor, aislamiento de configuración, puentes de canal/agente —, mientras que epic-harness (3 comandos, 26 habilidades de activación automática, que evoluciona a partir de tus patrones de fallo) aporta inteligencia de agente. Misma familia `epicsagas`; una separación limpia de responsabilidades entre modos.
...
- `claudy session sanitize`: repara sesiones con bloques thinking inválidos escritos por proveedores no-Anthropic.
- `claudy [profile] handoff [codex|agy] [-c|-r]`: continúa una sesión de codex/agy agotada en una nueva sesión de Claude.
...
## Continuidad de sesión entre proveedores

Al trabajar con un proveedor no-Anthropic (p. ej. Z.AI / GLM), el archivo JSONL de sesión contiene bloques thinking con firma vacía. Al reanudar esa sesión con la API de Anthropic se produce el siguiente error:

```
API Error: 400 Invalid `signature` in `thinking` block
```

Claudy gestiona esto de dos formas:

**Automática (puente de canal):** Cuando el servidor de canal reanuda una sesión, convierte silenciosamente los bloques thinking con firma vacía en bloques de texto. No se requiere ninguna acción.

**Manual (CLI):** Usa `claudy session sanitize` antes de reanudar con `claude --resume`:

```bash
# Interactivo — seleccionar de la lista de sesiones con problemas
claudy session sanitize

# Filtrar por nombre de proyecto
claudy session sanitize --project book-forge

# Procesar todas las sesiones de una vez
...
Frameworks como gstack, superpowers, ecc o nuestro [epic-harness](https://github.com/epicsagas/epic-harness) incluyen su propio `CLAUDE.md`, skills y agentes. Mantenlos aislados:
...
- **`400 Invalid signature in thinking block` al reanudar una sesión**: la sesión fue creada con un proveedor no-Anthropic (p. ej. Z.AI). Ejecuta `claudy session sanitize` para convertir los bloques thinking inválidos y luego reanuda normalmente.
