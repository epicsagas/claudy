...
| 🫴 | क्रॉस-CLI सेशन हैंडऑफ़ | Codex या Antigravity में कोटा खत्म? `claudy handoff` सेशन निकालकर नए Claude सेशन में सीड करता है |
| 🛡️ | एग्रेस गार्ड | `claudy --guard <profile>` — लोकल DLP प्रॉक्सी कुछ भी मशीन से बाहर जाने से पहले इमेज ब्लॉक हटाता है और लीक हुए सीक्रेट्स रीडैक्ट करता है |
| 🛠️ | संचालन UX | इंस्टॉल, अपडेट, अनइंस्टॉल, डॉक्टर, पिंग — सब कुछ एक बाइनरी से |

## Codex या Antigravity में उपयोग सीमा आ गई? सेशन Claude को सौंपें

क्रॉस-प्रोवाइडर रिज़्यूम केवल Claude CLI के अंदर काम करता है — Codex और Antigravity अपने सेशन अपने ही फ़ॉर्मैट में रखते हैं, इसलिए `--resume` उन्हें लोड नहीं कर सकता। जब इन CLI का कोटा खत्म हो जाता है, तो `claudy handoff` विदेशी सेशन से बातचीत का डाइजेस्ट निकालकर उसके साथ एक **नया** Claude सेशन सीड करता है:

```bash
# इंटरैक्टिव — मौजूदा डायरेक्टरी के codex + agy सेशन दिखाता है
claudy handoff

# किसी विशेष प्रोफ़ाइल के साथ (कोई भी प्रोफ़ाइल प्रीफ़िक्स चलेगा)
claudy zai handoff

# दोनों CLI में सबसे हाल का सेशन, बिना पिकर
claudy zai handoff -c

# 5 सबसे हाल के सेशनों में से चुनें
claudy handoff -r

# एक CLI तक सीमित; लॉन्च किए बिना डाइजेस्ट प्रीव्यू
...
`-c` Claude के `--continue` (सबसे हाल का) की तरह और `-r` `--resume` (चुनें) की तरह है — लेकिन विदेशी सेशन स्टोर पर। `--yolo` और अन्य कोई भी अनजान फ़्लैग Claude सेशन को वैसे ही वर्बैटिम फ़ॉरवर्ड होते हैं।

**क्या निकाला जाता है:** मूल यूज़र प्रॉम्प्ट (वर्बैटिम, कैप के साथ), असिस्टेंट रिप्लाई और टूल गतिविधि (एक-लाइनर तक ट्रंकेटेड), असिस्टेंट की अंतिम स्थिति, और वर्कस्पेस पाथ — 16 KiB बजट में एकल प्रॉम्प्ट में रेंडर। Codex की rollout फ़ाइलें नेटिव रूप से पार्स होती हैं। Antigravity का सेशन DB अनडॉक्यूमेंटेड फ़ॉर्मैट है, इसलिए claudy उसे बेस्ट-एफ़र्ट पढ़ता है (जेनेरिक protobuf स्ट्रिंग स्कैन प्लस प्रॉम्प्ट हिस्ट्री इंडेक्स) और लेआउट बदलने पर प्रॉम्प्ट-ओनली पर लौट आता है।

नया Claude सेशन उसी वर्किंग डायरेक्टरी में डाइजेस्ट को पहले मैसेज के रूप में लेकर शुरू होता है — यह बातचीत के सारांश और रेपो की मौजूदा स्थिति (`git status`/`git diff` सुझाए जाते हैं) की समीक्षा करके वहीं से जारी रहता है। यह कॉन्टेक्स्ट हैंडऑफ़ है, बिट-फ़ॉर-बिट रिज़्यूम नहीं: मॉडल से विवरण दोबारा सत्यापित करने की अपेक्षा रखें, रिप्ले नहीं।

**फ़्लैग:** `[codex|agy]` पोज़िशनल सोर्स (छोड़ने पर दोनों स्कैन) · `-c, --continue` (सबसे हाल का सेशन) · `-r, --resume` (5 सबसे हाल के में से चुनाव) · `--id <session-id>` (पिकर छोड़ें) · `--cwd <dir>` (डिफ़ॉल्ट: मौजूदा डायरेक्टरी; कुछ मैच न होने पर सभी सेशन पर फ़ॉलबैक) · `--profile <p>` (या प्रोफ़ाइल प्रीफ़िक्स: `claudy zai handoff`) · `--print` (केवल stdout) · `--yolo` (Claude को `--dangerously-skip-permissions` पास करें; अन्य अनजान फ़्लैग वर्बैटिम फ़ॉरवर्ड)।

## गेटवे तक सीक्रेट और इमेज जाने से रोकें: `--guard`

थर्ड-पार्टी गेटवे सब देखता है — पूरा सेशन प्लेन टेक्स्ट में, और कोई भी बाइनरी जिसे मॉडल को देखना है (स्क्रीनशॉट, लोकल इमेज रीड) base64 के रूप में अपलोड। कुछ गेटवे इन बाइनरी को बताए बिना अपने स्टोरेज बकेट में दोबारा अपलोड कर देते हैं। `--guard` Claude CLI और प्रदाता के बीच एक लोकल रिवर्स प्रॉक्सी लगाता है: रिक्वेस्ट बॉडी **मशीन से बाहर जाने से पहले** जाँची और रीराइट की जाती हैं, रिस्पॉन्स वैसे ही स्ट्रीम होकर लौटते हैं।

```bash
claudy --guard zai          # कोई भी पोज़िशन चलती है: claudy zai --guard भी
claudy --guard zai work     # मोड्स के साथ हमेशा की तरह संयोजित
```

शुरुआत पर claudy प्रिंट करता है कि प्रॉक्सी कहाँ लिसन कर रहा है:

```
  [claudy] guard active: 127.0.0.1:53467 -> https://api.z.ai/api/anthropic (images stripped, secrets scanned)
```

**हर रिक्वेस्ट पर क्या होता है:**

| पहचान | डिफ़ॉल्ट एक्शन | विवरण |
|---|---|---|
| `{"type":"image"}` कंटेंट ब्लॉक (मैसेज कंटेंट, `system` ऐरे, नेस्टेड `tool_result` कंटेंट) | टेक्स्ट प्लेसहोल्डर से रिप्लेस | बाइनरी गेटवे तक पहुँचती ही नहीं, दोबारा अपलोड असंभव |
| API कीज़ — `sk-*`, `AKIA…`, `gh[posu]_…`, Slack `xox[bpas]-…` | टोकन `[REDACTED:<kind>]` से रिप्लेस | कैरेक्टरसेट कोट्स/JSON स्ट्रक्चर को छोड़ते हैं, रिक्वेस्ट पार्सेबल रहती है |
| `Bearer`/`Basic` टोकन, `key=value` जोड़े | टोकन रिप्लेस, प्रीफ़िक्स सुरक्षित | न्यूनतम लंबाई गद्य के फ़ॉल्स पॉज़िटिव रोकती है |
| नॉन-JSON या अनपार्सेबल बॉडी | पास-थ्रू + लेजर वार्निंग | fail-open: स्कैनर की सीमाओं पर कभी ब्लॉक नहीं |

साफ़ रिक्वेस्ट बाइट-आइडेंटिकल फ़ॉरवर्ड होती हैं। हर रिक्वेस्ट `~/.claudy/guard/ledger.jsonl` में लॉग होती है (मेथड, पाथ, अपस्ट्रीम होस्ट, बाइट काउंट, फ़ाइंडिंग्स) — प्रीव्यू मास्कड होते हैं (`sk-a****f789`), रॉ सीक्रेट मटेरियल कभी नहीं लिखा जाता।

**पॉलिसी** `config.yaml` में `guard:` के अंतर्गत है:

```yaml
guard:
  strip_images: true          # एग्रेस से पहले इमेज ब्लॉक रिप्लेस
  on_secret: redact           # allow | redact | warn | block
  trusted_providers: [native] # अन्य पर फ़ाइंडिंग री-रूट सलाह जोड़ती है
```

`block` रिक्वेस्ट को 400 के साथ अस्वीकार करता है और अपस्ट्रीम से कभी संपर्क नहीं करता। `trusted_providers` अविश्वसनीय प्रदाता पर संवेदनशील कंटेंट मिलने पर एक बार की सलाह (stderr + लेजर) देता है — सेशन के बीच प्रदाता बदलना असंभव है, इसलिए यह सुझाव भर रहता है।

**सीमाएँ:** `--guard` के तहत विज़न जानबूझकर टूटा है (मॉडल प्लेसहोल्डर देखता है — यही उद्देश्य है); दुर्लभ फ़ॉल्स पॉज़िटिव वैध कंटेंट के अंदर की-जैसी स्ट्रिंग रीडैक्ट कर सकते हैं (जैसे बड़े base64 ब्लॉब के भीतर `AKIA`-जैसे रन); चैनल ब्रिज और `handoff` लॉन्च अभी गार्ड से नहीं गुज़रते।
...
- `claudy [profile] handoff [codex|agy] [-c|-r]`: कोटा खत्म हुआ codex/agy सेशन नए Claude सेशन में जारी रखता है।
